...
\section[
  status=published,
  tag=0SE01,
  label=\labelNFM{sec:section-preview}
...
\begin{definition}[
  ...
  status=published,
  tag=0SE0101,
  label=\labelNFM{def:section-preview}
]
...
\subsection[
  status=published,
  tag=0SE0102,
  label=\labelNFM{subsec:section-preview}
]{A tagged subsection}
...
\section[
  status=draft,
  tag=0SE02,
  label=\labelNFM{sec:section-gallery}
]{A draft environment gallery}

\subsection[
  tag=0SE0201,
  label=\labelNFM{subsec:section-gallery}
...
\begin{mainthm}[
  title={Standalone coherence},
  label=\labelNFM{mainthm:section}
]
  A standalone section can use the same source as its chapter and book builds.
\end{mainthm}

\begin{definition}[label=\labelNFM{def:section-draft}]
  A chain complex is a sequence \((C_n,d_n)\) with
  \(d_{n-1}\circ d_n=0\).
\end{definition}

\begin{proposition}[
  title={Cycles modulo boundaries},
  label=\labelNFM{prop:section}
]
  The quotient \(\ker d_n/\operatorname{im}d_{n+1}\) is an abelian group.
\end{proposition}

\begin{theorem}[label=\labelNFM{thm:section}]
...
\begin{proof}[Proof sketch of \cref{thm:section}]
  ...
  \begin{claim}[label=\labelNFM{claim:section}]
...
\subsection[
  tag=0SE0203,
  label=\labelNFM{subsec:section-features}
