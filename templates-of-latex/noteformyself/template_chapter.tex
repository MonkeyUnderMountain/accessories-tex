...
\section[
  status=published,
  tag=0CH01,
  label=\labelNFM{sec:chapter-preview}
...
\begin{theorem}[
  ...
  status=published,
  tag=0CH0101,
  label=\labelNFM{thm:chapter-preview}
]
...
\subsection[
  status=published,
  tag=0CH0102,
  label=\labelNFM{subsec:chapter-preview}
]{A tagged subsection}
...
\section[
  ...
  status=draft,
  tag=0CH02,
  label=\labelNFM{sec:chapter-gallery}
]{Draft environment gallery}

\subsection[
  tag=0CH0201,
  label=\labelNFM{subsec:chapter-gallery}
...
\begin{lemma}[
  ...
  label=\labelNFM{lem:chapter}
]
  Localization commutes with finite colimits of modules.
\end{lemma}

\begin{corollary}[label=\labelNFM{cor:chapter}]
  A localization of a finitely presented module is finitely presented.
\end{corollary}

\begin{conjecture}[label=\labelNFM{conj:chapter}]
  The comparison remains true after replacing schemes by suitable stacks.
\end{conjecture}

\begin{question}[
  title={Minimal hypotheses},
  label=\labelNFM{ques:chapter}
]
  Which finiteness assumption is actually used in the argument?
\end{question}

\begin{example}[tag=0CH0202,label=\labelNFM{eg:chapter}]
  The affine line is \(\mathbb{A}^1_k=\operatorname{Spec}(k[t])\). The provisional
  draft tag is hidden in both the heading and \cref{eg:chapter}.
\end{example}

\begin{exercise}[label=\labelNFM{ex:chapter}]
  Compute the standard-open cover of \(\mathbb{P}^1_k\).
\end{exercise}

\begin{construction}[
  title={Gluing two affine lines},
  label=\labelNFM{constr:chapter}
]
  Glue \(\operatorname{Spec}(k[t])\) and \(\operatorname{Spec}(k[t^{-1}])\)
  along \(\operatorname{Spec}(k[t,t^{-1}])\).
\end{construction}

\begin{notation}[label=\labelNFM{not:chapter}]
  Denote the resulting scheme by \(\mathbb{P}^1_k\).
\end{notation}

\begin{remark}[
  title={About draft references},
  label=\labelNFM{rmk:chapter}
]
...
\begin{proof}[Proof of \cref{mainthm:chapter}]
  ...
  \begin{claim}[label=\labelNFM{claim:chapter}]
...
\subsection[
  tag=0CH0203,
  label=\labelNFM{subsec:chapter-features}
